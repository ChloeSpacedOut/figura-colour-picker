...
textures['colourPicker.textures.hueSideSliderColour']:update()

function events.tick()
	if not colourPicker.isEnabled then return end
	models.colourPicker.GUI.HUD.colourPicker:setPos(-client.getScaledWindowSize().x/2,-client.getScaledWindowSize().y/2,0)

	for k,v in pairs(colourPicker.UI) do
		if models.colourPicker.GUI.HUD.colourPicker.highlights[k] ~= nil then
			models.colourPicker.GUI.HUD.colourPicker.highlights[k]:setColor(0.16,0.16,0.16)
		end
	end
	...
	if colourPicker.hoveredElement ~= nil then
		if colourPicker.UI[colourPicker.hoveredElement].isClicked == 1 then 
			if models.colourPicker.GUI.HUD.colourPicker.highlights[colourPicker.hoveredElement] ~= nil then
				models.colourPicker.GUI.HUD.colourPicker.highlights[colourPicker.hoveredElement]:setColor(0.4,0.4,0.4)
			end
		elseif colourPicker.UI[colourPicker.hoveredElement].isClicked == 0 and not colourPicker.selectedElement then
			
			if models.colourPicker.GUI.HUD.colourPicker.highlights[colourPicker.hoveredElement] ~= nil then
				models.colourPicker.GUI.HUD.colourPicker.highlights[colourPicker.hoveredElement]:setColor(0.32,0.32,0.32)
			end
		end
	end
	if colourPicker.selectedElement ~= nil then
		if colourPicker.UI[colourPicker.selectedElement].isClicked == 1 then
			if models.colourPicker.GUI.HUD.colourPicker.sliders[colourPicker.selectedElement..'X'] ~= nil or models.colourPicker.GUI.HUD.colourPicker.sliders[colourPicker.selectedElement..'Y'] ~= nil then
				local sliderOutput = {}
	...
	if colourPicker.colourSelectionData then
		models.colourPicker.GUI.HUD.colourPicker.highlights[colourPicker.colourSelectionData]:setColor(0.4,0.4,0.4)
	end
end

function events.render()
	if action_wheel:isEnabled() then
		models.colourPicker.GUI.HUD.colourPicker:setVisible(false)
	else
		models.colourPicker.GUI.HUD.colourPicker:setVisible(colourPicker.isEnabled)
	end
